...
public class MyLoginForm extends LoginForm {
  ...
  public MyLoginForm(VaadinAccessControl accessControl) {
    ...
    HorizontalLayout buttonLine = new HorizontalLayout(loginButton, new SimpleNavigationButton(Icon.get("user-add") + "Signup",
	...
	boolean result = MyLoginForm.this.accessControl.getLoginService().passwordResetRequest(getUserNameField().getValue());
	if (result) {
	  Notification.show("Please check your email", Type.HUMANIZED_MESSAGE);
	} else {
	  Notification.show("Could not sent password reset email", Type.ERROR_MESSAGE);
	}
      }
    });
